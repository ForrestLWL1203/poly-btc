...
PARAM_SPEC = [
    ...
    ("MAX_MARGIN_PCT",       "follow",  "yellow", "pct",     "immediate", config.MAX_MARGIN_PCT * 100,
        "每单基础保证金", "单笔基础保证金(占可用余额%),低波动币(σ≤基准)拿满此额;高波动币按波动率自动缩小(见下)。调高=每单更重、占满更快、能同时跟的单更少;调低=每单更轻、能跟更多单。实际值见顶部「模拟下单」"),
    ("MARGIN_SIGMA_REF",     "follow",  "yellow", "pct",     "immediate", config.MARGIN_SIGMA_REF * 100,
        "满仓波动率基准", "日波动率≤此的币(BTC/ETH等)拿满额保证金;高于此按「基准÷波动率」自动缩小(波动率2倍→保证金减半,风险平价)。调高=更多币拿满额、整体仓位更大;调低=只有最稳的币拿满、高波动币压更小"),
    ("STABLE_LEV_CAP",       "follow",  "yellow", "x",       "immediate", config.STABLE_LEV_CAP,
        "稳定币杠杆上限", "稳定币(σ≤4%,BTC/ETH等):照抄目标钱包的杠杆,但不超过此上限(默认20x)。杠杆由目标决定、我们只封顶"),
    ("VOLATILE_LEV_CAP",     "follow",  "yellow", "x",       "immediate", config.VOLATILE_LEV_CAP,
        "波动币杠杆上限", "meme/山寨/股票这类高波动币:照抄目标杠杆但不超过此上限(给插针留缓冲,默认5x)。调高=颠的币也敢上高杠杆、易被扫爆;调低=压更死更安全"),
    ...
    ("MARGIN_FLOOR_FRAC",    "follow",  "hidden", "pct",     "immediate", config.MARGIN_FLOOR_FRAC * 100, "保证金下限(占基础%,高波动币兜底)", ""),
