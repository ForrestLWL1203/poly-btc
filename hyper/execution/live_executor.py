...
class LiveExecutor:
    ...
    def _is_forced_liquidation(raw_json: str | None) -> bool:
        """Recognize only Hyperliquid's explicit self-account liquidation marker.

        Liquidation fills are exchange-authored reductions and have no strategy
        CLOID. Arbitrary/manual fills remain subject to the normal fail-closed
    ...
    def unmatched_account_fill_count(self) -> int:
        """Count unlinked, non-liquidation account fills for diagnostics."""
        with self._lock:
            return sum(
    ...
    def reconcile(self) -> dict:
        with self._lock:
            ...
            # authoritative exchange reduction. Only the explicit exchange
            # liquidation marker is accepted here; arbitrary/manual fills do
