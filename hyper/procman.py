...
observer and trigger scans INDEPENDENTLY —
  ...
  点采集   → start_scan()      (spawns ``python -m hyper.cli.discover scan``)
  启动跟单 → start_observer()  (spawns ``python -m hyper.cli.observe observe``)
...
_NEEDLE = {OBSERVER: "hyper.cli.observe", SCAN: "hyper.cli.discover"}
...
def _observe_argv(db_path):
    return [PYTHON, "-m", "hyper.cli.observe", "--db", db_path, "observe"]
...
def _repair_scan_state(db_path):
    ...
    try:
        r = subprocess.run(
            [PYTHON, "-m", "hyper.cli.discover", "--db", db_path, "repair-watchlist"],
