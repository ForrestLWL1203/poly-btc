...
PARAM_SPEC = [
    ...
    ("HARVEST_WEEK_PNL_MIN", "scanner", "hidden", "usd", "rescan", config.HARVEST_WEEK_PNL_MIN,
        "新钱包近7日 PnL 方向", "固定使用非负方向；不按绝对利润或ROI偏向大账户"),
    ("HARVEST_MONTH_PNL_MIN", "scanner", "hidden", "usd", "rescan", config.HARVEST_MONTH_PNL_MIN,
        "新钱包近30日 PnL 方向", "固定使用非负方向；Core/Challenger和持仓钱包始终刷新证据"),
    ...
    ("HARVEST_PERP_PNL_SHARE_MIN", "scanner", "hidden", "pct", "rescan",
        config.HARVEST_PERP_PNL_SHARE_MIN * 100, "旧Perp盈利占比",
        "仅兼容旧快照；Portfolio现在只确认近7日Perp成交量"),
    ...
    ("PRE_STRICT_QUEUE_MAX_N", "scanner", "black", "int", "rescan",
        config.PRE_STRICT_QUEUE_MAX_N, "Pre-strict队列上限",
        "完整扫描最多冻结32个钱包进入价格路径严格回放；与Core上限相互独立"),
    ...
    ("SOURCE_QUALITY_MAX_N", "scanner", "hidden", "int", "rescan",
        config.SOURCE_QUALITY_MAX_N, "旧源质量池上限", "仅兼容旧快照；Top40源评分边界已退役"),
    ("SOURCE_MIN_EPISODES_30D", "scanner", "black", "int", "rescan",
        config.SOURCE_MIN_EPISODES_30D, "源钱包30日最低完整回合", "边界不完整回合不计入样本或胜率"),
    ("SOURCE_MIN_EPISODE_WIN_RATE", "scanner", "hidden", "pct", "rescan",
        config.SOURCE_MIN_EPISODE_WIN_RATE * 100, "旧源钱包胜率", "仅兼容旧快照；新资格使用条件式彩票保护"),
    ("SOURCE_LOW_FREQ_MIN_EPISODES_30D", "scanner", "hidden", "int", "rescan",
        config.SOURCE_LOW_FREQ_MIN_EPISODES_30D, "旧强力低频样本下界", "仅兼容旧快照；低频特殊通道已退役"),
    ("SOURCE_LOW_FREQ_MAX_EPISODES_30D", "scanner", "hidden", "int", "rescan",
        config.SOURCE_LOW_FREQ_MAX_EPISODES_30D, "旧强力低频样本上界", "仅兼容旧快照；低频特殊通道已退役"),
    ("SOURCE_LOW_FREQ_MIN_EPISODE_WIN_RATE", "scanner", "hidden", "pct", "rescan",
        config.SOURCE_LOW_FREQ_MIN_EPISODE_WIN_RATE * 100, "旧强力低频胜率", "仅兼容旧快照；新资格使用PF和条件式彩票保护"),
    ...
    ("ROUGH_COPY_MIN_WIN_RATE", "scanner", "hidden", "pct", "rescan",
        config.ROUGH_COPY_MIN_WIN_RATE * 100, "旧粗略Copy胜率", "仅兼容旧快照；新资格使用PF和条件式彩票保护"),
    ...
    ("CORE_COPY_MIN_WIN_RATE", "scanner", "hidden", "pct", "rescan",
        config.CORE_COPY_MIN_WIN_RATE * 100, "旧严格Copy胜率", "仅兼容旧快照；新资格使用PF和条件式彩票保护"),
    ("CORE_PROFITABILITY_CONTRACT", "scanner", "black", "display", "rescan",
        "源与Copy 30日完整已平样本≥7；粗略Copy PF≥1.25，"
        "30d与最近7d保守收益均须盈利；"
    ...
    ("COPY_CATASTROPHIC_LIQUIDATION_LOSS_PCT", "scanner", "hidden", "display", "rescan",
        f"< {config.COPY_CATASTROPHIC_LIQUIDATION_LOSS_PCT * 100:g}%",
        "单次灾难清算硬上限",
        "固定资格政策；按该复制回合开仓动态权益归一，达到8%立即安全退出"),
