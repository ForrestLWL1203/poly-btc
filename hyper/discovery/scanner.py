...
def optimize_current_core_surface(db, *, stamp=None, apply=False) -> dict:
    """Full-tune the exact published Core, then run its bounded final margin calibration.

    This is an explicit migration/repair operation for a membership published on a fixed surface. It is
    deliberately separate from automatic generation formation and refuses to change the current Core set.
    """
    generation_id = selection.latest_published_generation(db)
    if not generation_id:
        raise RuntimeError("current_core_surface_optimization_requires_generation")
    meta = db.execute(
        "SELECT COALESCE(gmm.asof_ms,CAST(strftime('%s',sg.started_at) AS INTEGER)*1000) "
        "FROM scan_generation sg LEFT JOIN generation_market_manifest gmm "
        "ON gmm.generation=sg.generation WHERE sg.generation=? "
    ...
    now_ms = int(meta[0] or 0) if meta else 0
    if now_ms <= 0:
        raise RuntimeError("current_core_surface_optimization_asof_missing")
    expected_core = tuple(dict.fromkeys(
        str(addr or "").lower()
        for addr in (selection.published_core_membership(db) or ()) if addr
    ))
    if not expected_core:
        raise RuntimeError("current_core_surface_optimization_requires_core")
    expected_revision = strategy_revision.active_revision_id(db)
    stamp = stamp or now_iso()
    before = params.load_follow(db)
    base_follow = dict(before)
    scanner_values = params.load_category(db, "scanner")
    base_follow.update({
        key: scanner_values[key]
        for key in COPY_POLICY_PARAM_KEYS if key in scanner_values
    })
    if "SMART_ADD" in base_follow:
    ...
    exact = _retune_exact_membership_surface(
        db, expected_core, candidate_rows,
        generation_id=generation_id, stamp=stamp, round_index=1,
        now_ms=now_ms, base_follow=base_follow,
        valuation_marks=_current_copy_valuation_marks(),
        sigmas=auto_tune._load_sigmas(db, generation_id),
        market_ctx=auto_tune._load_market_ctx(db, generation_id),
    )
    ...
    if len(selected) != len(expected_core) or set(selected) != set(expected_core):
        raise RuntimeError(
            "current_core_surface_optimization_membership_not_certified:"
            f"{len(expected_core)}:{len(selected)}"
        )
    search = dict(formation.get("search") or {})
    calibration = dict(search.get("finalMarginCalibration") or {})
    if calibration.get("status") != "ok":
        raise RuntimeError(
            "current_core_surface_optimization_final_margin_failed:"
            f"{calibration.get('reason') or 'unknown'}"
        )
    proposal = dict(formation.get("params") or {})
    keys = (*auto_tune.TUNE_KEYS, *auto_tune.ADD_TUNE_KEYS)
    ...
    full_run = dict(exact.get("run") or {})
    tune_audit = {
        "status": "ok",
        "reason": exact.get("reason"),
        "tunedInputCount": len(expected_core),
        "searchProfile": full_run.get("search_profile") or "efficient",
        "quickReplayCount": full_run.get("quick_replay_count"),
        "strictFinalistCount": len(exact.get("finalistAudit") or ()),
    }
    revision = expected_revision
    if apply:
        db.commit()
        ...
        if selection.latest_published_generation(db) != generation_id:
            db.rollback()
            raise RuntimeError("current_core_surface_optimization_generation_changed")
        current_core = tuple(dict.fromkeys(
            str(addr or "").lower()
            for addr in (selection.published_core_membership(db) or ()) if addr
        ))
        if set(current_core) != set(expected_core) or len(current_core) != len(expected_core):
            db.rollback()
            raise RuntimeError("current_core_surface_optimization_membership_changed")
        current_follow = params.load_follow(db)
        if any(
            abs(f(current_follow.get(key)) - f(before.get(key))) > 1e-12
            for key in keys
            ...
            db.rollback()
            raise RuntimeError("current_core_surface_optimization_params_changed")
        _apply_formation_params(db, formation, stamp)
        active = strategy_revision.create_revision(
            db,
            generation_id,
            source="current_core_full_surface_calibration",
            parent_revision=expected_revision,
            expected_active_revision=expected_revision,
            validation={
                "algorithm": "current_core_full_surface_calibration_v1",
                "coreCount": len(expected_core),
                "fullSurfaceTune": tune_audit,
                "finalMarginCalibration": calibration,
                "tierEconomics": search.get("tierEconomics") or {},
                "deploymentUtilization": search.get("finalDeploymentUtilization") or {},
                "resourcePeak": search.get("resourcePeak") or {},
            },
            reason="strict_current_core_full_surface_calibration",
            stamp=stamp,
        )
        revision = active["revision"]
        pipeline_audit._insert_event(
    ...
    _set_scan_progress(
        db, state="idle", stage="current_core_full_surface_calibration_complete",
        candidates_scanned=len(expected_core), candidates_total=len(expected_core),
    )
    db.commit()
    return {
        ...
        "deploymentUtilization": search.get("finalDeploymentUtilization") or {},
        "tierEconomics": search.get("tierEconomics") or {},
        "finalMarginQuickReplayCount": calibration.get("quick_replay_count"),
        "finalMarginStrictFinalistCount": len(calibration.get("finalists") or ()),
        "resourcePeak": search.get("resourcePeak") or {},
        "portfolioReplay": portfolio_replay,
    }
