import { TIER_GROUPS, WALLET_SIDE_CAPS } from "./paramMeta.js";
...
export function validateFollowParams(vals) {
  ...
  WALLET_SIDE_CAPS.forEach(({ key, label }) => {
    const okSide = validatePct(label, key);
    if (okWallet && okSide && numVal(key) > numVal("WALLET_MARGIN_CAP_PCT")) {
      markErr(`${label}不能高于单钱包总保证金上限`, [key, "WALLET_MARGIN_CAP_PCT"]);
    }
  });
