...
React UMD + precompiled `app.js`. Edit `app.jsx`, then run `web/build.sh` before deployment.
...
├── app.js          precompiled bundle generated by web/build.sh
...
1. **Vendor libs**. React/ReactDOM are committed because `index.html` needs them on the VPS:
   ```
   mkdir -p web/vendor && cp /path/to/poly-fight/poly_fight/dashboardV2/vendor/{react-18.3.1.production.min.js,react-dom-18.3.1.production.min.js} web/vendor/
   ```
